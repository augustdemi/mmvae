...
from model import *
from loss import kl_loss_function
import json
...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        if self.ckpt_load_iter == 0:  # create a new model
            ...
            self.encoderB = EncoderA(self.zB_dim, self.zS_dim, channel=3)
            self.decoderA = DecoderA(self.zA_dim, self.zS_dim)
            self.decoderB = DecoderA(self.zB_dim, self.zS_dim, channel=3)
            self.alpha = Alpha()
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter+1):
            ...
            loss_kl_infB = kl_cont_loss_infA
            ...
            loss_kl_infA_givenB = cont_capacity_loss_infA
            loss_kl_infB_givenA = disc_capacity_loss_infA
            ##########################################################################################################################

            loss_kl = capacity_loss_infA
            ...
            loss_recon_infA = F.binary_cross_entropy(torch.sigmoid(XA_infA_recon), XA, reduction='sum').div(XA.size(0))
            #
    ...
    def save_traverse(self, iters, logalpha, logalphaA, logalphaB, z_A, z_B, z_S, loc=-1):
        ...
        if self.record_file:
            ...
            for i, idx in enumerate(fixed_idxs):
                Z['fixed' + str(i)] = \
                    [fixed_zmuA[i], fixed_zmuB[i], fixed_cate_probS[i]]
            Z['random'] = [random_zmuA, random_zmuB, random_cate_probS]
