...
from utils import DataGather, mkdirs, grid2gif, apply_poe, sample_gaussian, sample_gumbel_softmax, kl_multiple_discrete_loss
from model import *
...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        if self.ckpt_load_iter == 0:  # create a new model
            ...
            self.encoderB = EncoderB(self.zB_dim, self.zS_dim, channel=3)
            self.decoderA = DecoderA(self.zA_dim, self.zS_dim)
            self.decoderB = DecoderB(self.zB_dim, self.zS_dim, channel=3)
            self.alpha = Alpha()
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter+1):
            ...
            (kl_cont_loss_infA, kl_disc_loss_infA, cont_capacity_loss_infA, disc_capacity_loss_infA) = kl_loss_function(self.use_cuda, iteration, latent_dist_infA)
            ...
            (kl_cont_loss_infB, kl_disc_loss_infB, cont_capacity_loss_infB, disc_capacity_loss_infB) = kl_loss_function(self.use_cuda, iteration, latent_dist_infB)

            loss_kl_infB = kl_cont_loss_infB + kl_disc_loss_infB
            capacity_loss_infB = cont_capacity_loss_infB + disc_capacity_loss_infB

            loss_kl_POE = kl_disc_loss_infA
            ...
            loss_kl_infA_givenB = cont_capacity_loss_infB
            loss_kl_infB_givenA = disc_capacity_loss_infB
            ##########################################################################################################################

            loss_kl = capacity_loss_infB
            ...
            loss_recon_infA = reconstruction_loss(XA, torch.sigmoid(XA_infA_recon))
            #
            loss_recon_infB = reconstruction_loss(XB, torch.sigmoid(XB_infB_recon), distribution=RECON_DIST[2])
            #
            loss_recon_POE = \
            ...
            # total loss for vae
            # vae_loss = loss_recon + self.lamkl * loss_kl + self.beta1 * tcA_loss
            ...
            # zA, zS = encA(xA)
            muA_infA2, stdA_infA2, _, cate_prob_infA2 = self.encoderA(XA2)

            # zB, zS = encB(xB)
            muB_infB2, stdB_infB2, _, cate_prob_infB2 = self.encoderB(XB2)

            ZA_infA2 = sample_gaussian(self.use_cuda, muA_infA2, stdA_infA2)
            ZB_infB2 = sample_gaussian(self.use_cuda, muB_infB2, stdB_infB2)

            # encoder samples (for cross-modal prediction)
            ZS_infA2 = sample_gumbel_softmax(self.use_cuda, cate_prob_infA2)
            ZS_infB2 = sample_gumbel_softmax(self.use_cuda, cate_prob_infB2)
    ...
    def save_recon2(self, iters, index, XA, XB,
                    XA_infA_recon, XB_infB_recon, XA_POE_recon, XB_POE_recon, muA_infA, muB_infB, muS_infA, muS_infB,
                    muS_POE,
                    logalpha, logalphaA, logalphaB):
    ...
    def get_stat(self, logalpha, logalphaA, logalphaB):
        ...
        for _ in range(10000):
            ...
            random_zmuA, _, _, cate_prob_infA = self.encoderA(random_XA)

            random_zmuB, _, _, cate_prob_infB = self.encoderB(random_XB)

            random_cate_prob_POE = torch.exp(
                torch.log(torch.tensor(1 / 10)) + torch.log(cate_prob_infA) + torch.log(cate_prob_infB))


            z_A.append(random_zmuA.cpu().detach().numpy()[0])
            z_B.append(random_zmuB.cpu().detach().numpy()[0])
            z_S.append(random_cate_prob_POE.cpu().detach().numpy()[0])
        return z_A, z_B, z_S
    ...
    def save_traverse(self, iters, logalpha, logalphaA, logalphaB, z_A, z_B, z_S, loc=-1):
        ...
        random_XB = random_XB.unsqueeze(0)
        #
        random_zmuA, _, _, cate_prob_infA = encoderA(random_XA)

        # zB, zS = encB(xB)
        random_zmuB, _, _, cate_prob_infB = encoderB(random_XB)


        # zS = encAB(xA,xB) via POE
        random_cate_probS = torch.exp(
            torch.log(torch.tensor(1 / 10)) + torch.log(cate_prob_infA) + torch.log(cate_prob_infB))
        ...
        if self.record_file:
            ...
            # fixed_idxs = [12306, 3246, 21444, 31000, 40000]
            fixed_idxs = [3246, 7000, 14305, 19000, 27444, 33100, 38000, 45231, 51000, 55121]
            ...
            fixed_cate_probS = [0] * len(fixed_idxs)
            ...
            for i, idx in enumerate(fixed_idxs):
                ...
                fixed_zmuA[i], _, _, cate_prob_infA = encoderA(fixed_XA[i])

                # zB, zS = encB(xB)
                fixed_zmuB[i], _, _, cate_prob_infB = encoderB(fixed_XB[i])

                # zS = encAB(xA,xB) via POE
                fixed_cate_probS[i] = torch.exp(
                    torch.log(torch.tensor(1 / 10)) + torch.log(cate_prob_infA) + torch.log(cate_prob_infB))
            ...
            IMG['random'] = torch.cat(
                [torch.stack([each_XA, each_XA, each_XA]).unsqueeze(0), random_XB], dim=2)

            Z = {}
            for i, idx in enumerate(fixed_idxs):
                fixed_zS = sample_gumbel_softmax(self.use_cuda, fixed_cate_probS[i], train=False)
                Z['fixed' + str(i)] = \
                    [fixed_zmuA[i], fixed_zmuB[i], fixed_zS]
            random_zS = sample_gumbel_softmax(self.use_cuda, random_cate_probS, train=False)
            Z['random'] = [random_zmuA, random_zmuB, random_zS]
        ...
        gifs = torch.cat(gifs)
        gifs = gifs.view(
            len(Z), 1 + self.zA_dim + 1 + 1 + 1 + self.zB_dim,
        ...
        for i, key in enumerate(Z.keys()):
            for j, val in enumerate(interpolationA):
                ...
                save_image(
                    ...
                    nrow=1 + 1 + self.zA_dim + 1 + 1 + 1 + self.zB_dim,
                    pad_value=1)
            # make animated gif
            grid2gif(
