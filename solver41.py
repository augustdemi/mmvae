...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        if self.viz_on:
            self.win_id = dict(
                recon='win_recon', kl='win_kl', capa='win_capa', tc='win_tc', mi='win_mi', dw_kl='win_dw_kl'
            )
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter + 1):
            ...
            log_pzA, log_qzA, log_prod_qzAi, log_q_zACx = get_log_pz_qz_prodzi_qzCx(ZB_infB, (muB_infB, logvarB_infB),
            ...
            miA_loss = (log_q_zACx - log_qzA).mean()

            # TC[z] = KL[q(z)||\prod_i z_i]
            tcA_loss = (log_qzA - log_prod_qzAi).sum(dim=0).div(XA.size(0))

            # dw_kl_loss is KL[q(z)||p(z)] instead of usual KL[q(z|x)||p(z))]
            dw_klA_loss = (log_prod_qzAi - log_pzA).mean()
            #================================================================
            ...
            # total loss
            mi_loss = miA_loss + miS_loss
            tc_loss = tcA_loss
            dw_kl_loss = dw_klA_loss + dw_klS_loss
