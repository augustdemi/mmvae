...
class Solver(object):
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter+1):
            ...
            loss_kl_infA += kl_multiple_discrete_loss(self.use_cuda, [cate_prob_infA])

            loss_kl_infB = -0.5 * (
                (1 + logvarB_infB - muB_infB ** 2 - stdB_infB ** 2).sum(1).mean()
            )

            loss_kl_infB += kl_multiple_discrete_loss(self.use_cuda, [cate_prob_infB])



            loss_kl_infB = kl_multiple_discrete_loss(self.use_cuda, [cate_prob_infA])

            loss_kl_POE = kl_multiple_discrete_loss(self.use_cuda, [cate_prob_POE])
            ...
            loss_kl_infB_givenA = kl_multiple_discrete_loss(self.use_cuda, [cate_prob_infA])
            ##########################################################################################################################

            loss_kl = loss_kl_infA
    ...
    def save_traverse(self, iters, logalpha, logalphaA, logalphaB, z_A, z_B, z_S, loc=-1):
        ...
        decoderA = self.decoderA
        decoderB = self.decoderB
        interpolationA = torch.tensor(np.linspace(-4, 4, 30))
        interpolationB = torch.tensor(np.linspace(-4, 4, 30))
        interpolationS = torch.tensor(np.linspace(-4, 4, 30))
